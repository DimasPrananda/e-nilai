@php
    ...
    $perPage = $ranking_subcriterias->perPage();
...
@forelse($ranking_subcriterias as $index => $ranking_subcriteria)
    <tr class="border-b border-gray-300 dark:border-gray-700 dark:hover:bg-gray-800 hover:bg-gray-100 transition-colors duration-200">
        ...
        <td class="text-center">{{ $ranking_subcriteria->name }}</td>
        <td class="py-2">{{ $ranking_subcriteria->detail }}</td>
        <td class="text-center py-2">{{ $ranking_subcriteria->ranking_criteria->name }}</td>
        <td class="text-center py-2">
            <form action="{{ route('subcriteria.destroy', $ranking_subcriteria->id) }}" class="inline-block relative group" method="POST" onsubmit="return confirm('Are you sure you want to delete this sub kriteria?');">
                @csrf
                @method('DELETE')
...
<tr>
    <td colspan="7" class="text-center py-4" id="pagination-links">
        {{ $ranking_subcriterias->appends(request()->query())->links() }}
    </td>
</tr>