<x-app-layout>
    <div class="p-4 md:p-12 flex-1 " x-data="{ open: false }">
        ...
        <div class="md:p-0 mb-4">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-2">Kelola Data Kriteria Penilaian</h1>
            <a href="{{ route('admin.dashboard') }}" class="text-gray-800 dark:text-gray-400 hover:dark:text-gray-200 hover:text-black">dashboard </a><a class="text-gray-800 dark:text-gray-200 font-bold">/ Kelola Data Kriteria Penilaian</a>
        </div>
        
        <!-- Formulir Input Kriteria -->
        <form class="mb-4" action="{{ route('criterias.store') }}" method="POST" x-show="open" x-transition>
            @csrf
            <div class="mb-4">
                ...
                <table class="flex-1">
                    <thead class="border-b-2 text-gray-800 dark:text-gray-200">
                        <tr>
                            <th>No</th>
                            <th>Nama Kriteria</th>
                            <th class=" w-96">Penjelasan / Detail</th>
                            <th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="text-gray-800 dark:text-gray-200 dark:bg-gray-800 bg-white">
                        ...
                        <tr class=" border-b border-gray-300 dark:border-gray-700">
                            ...
                            <td class="py-2">{{ $criteria->name }}</td>
                            <td class="py-2">{{ $criteria->detail }}</td>
                            <td class="text-center py-2">
                                <form action="{{ route('criterias.destroy', $criteria->id) }}" class="inline-block relative group" method="POST" onsubmit="return confirm('Are you sure you want to delete this criteria?');">
                                    @csrf
                                    @method('DELETE')
